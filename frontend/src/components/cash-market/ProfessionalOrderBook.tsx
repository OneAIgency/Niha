...
export function ProfessionalOrderBook({ orderBook, onPriceClick, showFullBook = false }: ProfessionalOrderBookProps) {
  ...
  return (
    <div className="panel panel--flush text-xs flex flex-col h-full">
      {/* Column Headers */}
      <div className="grid grid-cols-1 md:grid-cols-2 border-b border-navy-700 flex-shrink-0">
        {/* Bids Header */}
        <div className="px-4 py-1.5 border-r border-navy-700">
          <div className="grid grid-cols-3 md:grid-cols-4 gap-2 text-xs font-medium text-navy-400">
      ...
      >
        {Array.from({ length: lineCount }, (_, lineIdx) => {
          ...
          const lineBg =
            lineIdx % 2 === 1 ? 'bg-navy-500/15' : '';
          return (
            <div
              key={lineIdx}
              className={`grid grid-cols-1 md:grid-cols-2 border-b border-navy-800/50 ${lineBg}`}
            >
              {/* Bid cell */}
              <div className="border-r border-navy-700 min-h-0">
                {bidLevel ? (
                  <OrderBookRow
      ...
      {showFullBook && (
        <div className="px-4 py-2 border-t border-navy-700 bg-navy-900/30 flex-shrink-0">
          <div className="flex items-center justify-between text-xs text-navy-400">
            <span>
              <span className="font-semibold text-emerald-400">{orderBook.bids.length}</span> bid levels
            </span>
            <span>
              <span className="font-semibold text-red-400">{orderBook.asks.length}</span> ask levels
            </span>
          </div>
        </div>
      )}
