...
export const HERO_METRICS = [
  ...
  { value: 'T+2–T+5', label: 'OTC Settlement', color: 'blue' as const, depth: 'essential' as ContentDepth },
  // Advanced
...
export const OVERVIEW_NARRATIVES: { id: string; title: string; content: string; depth: ContentDepth }[] = [
  {
    id: 'core-thesis',
    title: 'Core Thesis',
    content: 'This confidential briefing is prepared exclusively for NIHA\'s introducer partners. The EU and China operate the world\'s two largest carbon markets, yet they are completely disconnected — different registries, currencies, regulations, and access requirements. This structural gap creates a persistent 7–10× price differential. NIHA, based in Hong Kong, is the only entity with simultaneous access to both markets, enabling CEA→EUA swaps that deliver 8–12% savings to EU compliance entities.',
    depth: 'essential',
  },
  {
    id: 'what-this-means',
    title: 'What This Means for Introducers',
    content: 'The OTC carbon market handles 15–25% of EU ETS volume — a massive segment operating outside exchanges. These participants value confidentiality, block execution, and price improvement — exactly what NIHA offers. As an introducer, you\'re connecting clients to the only bridge between these two markets at a time when the opportunity window is at its widest.',
    depth: 'advanced',
  },
  {
    id: 'otc-opportunity',
...
export const HOW_IT_WORKS_STEPS = [
  ...
  },
  {
    ...
    extendedContent: 'NIHA leverages its exclusive Greater Bay Area access and established relationships with Chinese compliance entities to source CEA bilaterally. Counterparties with surplus allowances — power generators, new-entrant steel/cement/aluminium facilities — commit (reserve) CEA availability ahead of each transaction. Actual CEA settlement completes T+2 to T+3 after trade execution, following standard pilot market bilateral clearing.',
  },
  {
    icon: ArrowRightLeft,
    title: 'CEA → EUA Swap',
    subtitle: 'Cross-border exchange at favorable ratio',
    extendedContent: 'The swap is executed with NIHA acting as principal (required by regulation — NIHA must take beneficial ownership of CEA). EUA for delivery are sourced from NIHA\'s counterparty network — entities who commit (reserve) EUA availability at agreed terms. NIHA does not warehouse EUA inventory; instead, it matches bilateral flows. The conversion ratio reflects the 7–10× price gap between markets, passing 8–12% savings to the client.',
  },
  {
    icon: CheckCircle,
    title: 'EUA Delivered',
    subtitle: 'Client receives EUA at 8–12% discount',
    extendedContent: 'EUA are delivered to the client\'s EU registry account — the same account they would use for any exchange purchase. Settlement follows standard bilateral OTC timelines (T+2 to T+5 from trade date), which is faster than the multi-leg exchange route (sell CEA domestically + buy EUA on ICE/EEX, each leg T+2, totalling 5-10 business days). The client retains full documentation for audit purposes.',
  },
  {
    icon: Shield,
...
export const DETAILED_EXAMPLES: { id: string; title: string; scenario: string; breakdown: string[]; saving: string }[] = [
  ...
  },
  {
    ...
    breakdown: [
      ...
      'Via NIHA: single bilateral swap, known ratio, T+2–T+5',
...
export const COMPARISON_ROWS = [
  ...
  { feature: 'Settlement', direct: 'T+2 per leg (5-10d total)', niha: 'T+2–T+5 single agreement', nihaWins: true, depth: 'essential' as ContentDepth },
  { feature: 'Block execution', direct: 'Hours, with slippage', niha: 'Pre-agreed price, single counterparty', nihaWins: true, depth: 'essential' as ContentDepth },
...
export const VALUE_PROPOSITIONS = [
  { title: '8–12% Cost Savings', description: 'Via CEA→EUA swap through exclusive cross-border market access', extendedDescription: 'The savings are structural, not speculative — they exist because two large, liquid markets are completely disconnected by regulation, currency, and access barriers. As long as the price gap persists (forecast through at least 2030), the savings are repeatable through NIHA\'s exclusive access.', color: 'emerald' as const, depth: 'essential' as ContentDepth },
  ...
  { title: 'Faster Settlement', description: 'Single OTC agreement vs multi-leg exchange execution', extendedDescription: 'Buying EUA via exchanges requires T+2 clearing per leg. The equivalent multi-step path (sell CEA domestically, convert CNY→EUR, buy EUA on ICE) takes 5-10 business days with multiple counterparties. Via NIHA: one bilateral agreement, one counterparty, T+2 to T+5 total — with price locked at trade date. For entities managing compliance deadlines, this simplicity and speed advantage is significant.', color: 'emerald' as const, depth: 'essential' as ContentDepth },
  // Advanced
...
export const CHINA_ETS_ACCORDIONS = [
  ...
  { id: 'access', title: 'Access Restrictions', content: 'Only onshore compliance entities can directly participate in China\'s national ETS. Foreign entities face prohibitive barriers: no registry access, no cross-border settlement capability, and no institutional market access pathway. Even pilot markets (Shenzhen, Shanghai) that theoretically allow foreign participation require months of setup with uncertain outcomes.', depth: 'essential' as ContentDepth },
...
export const CLIENT_PATHS = [
  {
    ...
    comparison: { via: 'Exchange: €81/t, T+2 per leg, visible on orderbook', niha: 'NIHA: €71-74/t, T+2–T+5 single agreement, zero market impact' },
    extendedExample: {
    ...
    detailedComparison: {
      rows: [
        ...
        { label: 'Settlement', exchange: 'T+2 per leg (5-10d multi-leg)', niha: 'T+2–T+5 bilateral' },
  ...
  },
  {
    ...
    label: 'Path C',
    color: 'blue' as const,
    keyPoint: 'Single bilateral transaction — no FX conversion needed',
    steps: ['Entity deposits EUA', 'NIHA swap engine executes', 'Entity receives CEA', 'No double exchange fees'],
    comparison: { via: 'Via exchanges: sell EUA + buy CEA, double fees, days', niha: 'NIHA: single bilateral swap, T+2–T+5' },
    extendedExample: {
      ...
      lines: [
        ...
        'Via NIHA: single bilateral swap at known ratio, T+2–T+5, ~1% total cost',
      ],
    },
    detailedComparison: {
      rows: [
        { label: 'Execution', exchange: 'Two separate trades', niha: 'Single bilateral swap' },
        ...
        { label: 'Time to complete', exchange: '5-10 business days', niha: 'T+2–T+5 (single agreement)' },
        { label: 'Execution risk', exchange: 'Leg risk between trades', niha: 'Single agreement — all or nothing' },
      ],
    },
  },
];
...
export const LEGAL_SUMMARY_CARDS = [
  ...
  },
  {
    ...
    description: 'Open to authorized foreign institutional investors through government-facilitated pathways — application required',
  },
  {
    label: 'NIHA (Hong Kong)',
    status: 'Authorized',
    statusColor: 'emerald' as const,
    description: 'GBA-zone entity with government-facilitated access to both Chinese pilot markets and European carbon markets',
  },
];

export const NIHA_AUTHORIZATION_CHAIN = [
  { step: 'Access', label: 'Cross-border Authorization', detail: 'Government-facilitated carbon trading pathway' },
  { step: 'Market', label: 'Pilot Market Access', detail: 'Institutional access via GBA framework' },
  { step: 'Settlement', label: 'Cross-border Settlement', detail: 'RMB/EUR bilateral clearing' },
  { step: 'Data', label: 'Regulatory Compliance', detail: 'GBA data governance framework' },
  { step: 'Delivery', label: 'EUA Delivery Capability', detail: 'European carbon allowance delivery' },
];
...
export const LEGAL_ACCORDIONS = [
  // Essential — Core legal argument
  {
    ...
    content: 'China operates two separate carbon trading systems with fundamentally different access rules. The National ETS (Shanghai SEEE) is closed by law to ALL foreign entities — including Hong Kong. Only ~3,500 onshore compliance entities (power, steel, cement, aluminium sectors) can trade national CEA under the State Council\'s Interim Regulations (May 2024). However, China also operates regional pilot markets — Shenzhen (since 2013), Guangdong/CEEX, Hubei, and others — that have been open to authorized foreign institutional investors through government-facilitated pathways. NIHA accesses Chinese carbon credits through these pilot markets, not the national ETS. The price differential works identically: all Chinese carbon credits (national and pilot) trade at Chinese price levels (~¥80-100/tonne, equivalent to ~€9-11/tonne) versus EU EUA at ~€81/tonne — a 7-10× differential.',
    depth: 'essential' as ContentDepth,
  },
  {
    id: 'eu-barriers',
    title: 'Three Barriers Blocking EU Entities from Chinese Carbon',
    content: 'Barrier 1 — National ETS Closed by Law: China\'s Interim Regulations on Carbon Emission Allowance Trading (State Council, effective May 2024) limit participation to "Key Emission Entities" — onshore companies covered by MRV requirements. The Shanghai SEEE explicitly prohibits brokerage services for foreign investors. No timeline exists for opening to other participants. Barrier 2 — Pilot Markets Practically Impossible: Even on pilot markets that technically allow foreign investors (Shenzhen, Guangdong/CEEX, Hubei), an EU entity would need: (1) NRA-RMB bank account at a Chinese bank — months of setup with no existing banking relationship, (2) Government foreign exchange authorization — no precedent for EU-based applicants in carbon trading, (3) Institutional market access — untested for EU entities, (4) PIPL data compliance — full CAC Security Assessment required, 6-12 months with no GBA exemption. No EU entity has ever successfully navigated this pathway — NIHA\'s access is facilitated through exclusive government arrangements. Barrier 3 — Legal Uncertainty: Chinese carbon credits are not classified as financial instruments, property rights, securities, or commodities under Chinese law. For an EU entity without local legal infrastructure, this undefined status creates unacceptable counterparty and insolvency risk.',
    depth: 'essential' as ContentDepth,
  },
  {
    id: 'why-hk',
    title: 'Why Hong Kong — And Only Hong Kong — Can Bridge Both Markets',
    content: 'Hong Kong is not mainland China, but it is part of the Greater Bay Area (GBA) — a special economic zone that provides privileged access no other foreign jurisdiction enjoys. Six specific advantages: (1) GBA Zone Access — part of the Guangdong-HK-Macao Greater Bay Area, with explicit carbon trading integration (HKEX Quadripartite MoU, September 2025), (2) Pilot Market Access — government-facilitated access for authorized GBA entities, with institutional-grade trading capability, (3) Cross-border Settlement — established banking channels for RMB/EUR bilateral clearing, (4) EU ETS Access — non-EU entities can participate in European carbon markets under established regulatory frameworks, (5) Common Law Jurisdiction — internationally recognized contract enforcement, ISDA/EFET-compatible legal framework, (6) Physical Proximity — 30 minutes from Shenzhen by high-speed rail, same time zone, shared economic infrastructure.',
    depth: 'essential' as ContentDepth,
  },
  {
    id: 'niha-authorizations',
    title: 'Not Any HK Entity: NIHA\'s Required Authorizations',
    content: 'Being a Hong Kong entity is necessary but not sufficient. Access to China\'s pilot carbon markets requires a specific set of authorizations that take months to obtain and demand ongoing compliance. Accessing Chinese pilot markets through Hong Kong requires: (1) Cross-border trading authorization — government-facilitated access for carbon emissions trading within the GBA framework, (2) Institutional market access — qualification as authorized participant on pilot exchanges, subject to review and ongoing compliance, (3) Cross-border settlement capability — dedicated bilateral clearing channels for RMB/EUR transactions, (4) Data governance compliance — GBA framework for cross-boundary information flow, (5) Ongoing regulatory compliance — adherence to pilot market rules, reporting obligations, and transaction transparency requirements. This authorization chain represents a significant first-mover advantage — a new competitor would need months of regulatory groundwork before executing a single trade.',
    depth: 'essential' as ContentDepth,
  },
  {
    id: 'transaction-architecture',
    title: 'Transaction Architecture: The 5-Step Custody Flow',
    content: 'Step 1 (Client Funds): EU entity deposits EUR into NIHA\'s segregated client account in Hong Kong — funds held separately from operations under HK law. Step 2 (Carbon Acquisition): NIHA acquires Chinese carbon credits from pre-committed counterparties via pilot markets, paying in RMB through established cross-border settlement channels. NIHA buys as principal — it owns the credits, not the EU client. CEA settlement follows standard pilot market bilateral clearing (T+2 to T+3). Step 3 (Custody): Credits held in NIHA\'s registry account on the pilot exchange. The EU entity cannot hold them — Chinese carbon registries are accessible only through government-facilitated institutional pathways. Custody is a regulatory necessity, not a choice. Step 4 (Swap Execution): NIHA executes the economic swap — retires or sells the Chinese credits and delivers EUA sourced from its counterparty network (entities who have committed EUA availability at agreed terms) at the agreed ratio. Step 5 (EUA Delivery): EUA transferred to the EU entity\'s European carbon registry account (T+3 to T+5 from trade date). The EU entity receives compliant EUA at 8-12% below market price. They never touch Chinese carbon credits and never interact with Chinese regulation.',
    depth: 'essential' as ContentDepth,
  },
  // Advanced
  {
  ...
  // Expert
  {
    ...
    content: 'Step 1 (EUR Deposit): Client funds held in segregated account at licensed Hong Kong bank, separated from NIHA\'s operational capital. Legal basis: HK common law client fund protection + bank segregation requirements. Step 2 (Carbon Credit Acquisition): NIHA acquires as principal, not agent — required because foreign entities cannot hold Chinese carbon registry accounts. Legal basis: Government-facilitated access for authorized GBA entities under established pilot market frameworks. Step 3 (Custody): Credits registered in NIHA\'s name on pilot exchange registry. Beneficial ownership documented in bilateral contract governed by HK law, but legal title with NIHA. This mirrors prime brokerage: the broker holds legal title to securities while the client retains beneficial ownership and economic exposure. Step 4 (Swap Execution): NIHA converts Chinese carbon credit economic value to EUA as principal transaction. EUA are sourced from committed counterparties, not pre-held inventory. Physical delivery of commodity — not a derivative under MiFID II (no cash settlement, actual EUA delivered). Not regulated under HK SFO Schedule 5 (carbon not classified as security or future). Step 5 (EUA Delivery): Standard European carbon registry transfer to client\'s account (T+3 to T+5 from trade date). Open to non-EU entities under Directive 2003/87/EC Art. 19. Client receives EUA with full provenance documentation and compliance trail.',
    depth: 'expert' as ContentDepth,
  },
  {
    id: 'authorization-detail',
    title: 'Complete Authorization Chain Across Five Jurisdictions',
    content: 'Accessing cross-border carbon markets requires authorization across multiple regulatory domains. China — Cross-border Trading: Government-facilitated access for carbon emissions trading within the GBA framework, covering foreign exchange clearance and institutional market participation. China — Institutional Access: Qualification as authorized participant on pilot exchanges, subject to ongoing compliance and reporting obligations. China — Settlement: Established bilateral clearing channels for cross-border RMB transactions, purpose-built for carbon trading flows. China — Data Governance: GBA framework for cross-boundary information flow — the simplified compliance pathway available exclusively to GBA-registered entities. EU — Carbon Market Access: Participation in European carbon markets under established regulatory frameworks that permit non-EU entities. Hong Kong — Regulatory Position: Carbon credits are not classified as regulated financial instruments under Hong Kong securities law, enabling carbon-focused operations without additional licensing requirements.',
    depth: 'expert' as ContentDepth,
  },
  {
    id: 'citations',
    title: 'Complete Regulatory Citation Index',
    content: 'China National ETS: Interim Regulations on Carbon Emission Allowance Trading (State Council, effective May 1, 2024) — Art. 12 limits to Key Emission Entities. MEE Decree No. 19 Art. 7 (covered entities), Art. 10 (allocation), Art. 14 (trading restrictions). China Pilot Markets: Government-facilitated access frameworks for authorized foreign institutional investors on pilot exchanges — no quota or currency restrictions for qualified GBA entities. Regional pilot market regulations for Shenzhen, Guangdong/CEEX, and Hubei each provide distinct access pathways. China Settlement: Established cross-border settlement frameworks for carbon trading, including dedicated bilateral clearing channels for RMB/EUR transactions. China Data: GBA Standard Contract for Cross-Boundary Flow of Personal Information (CAC + HK ITIB + PCPD, 2024). EU ETS: Directive 2003/87/EC Art. 12 (transfer of allowances), Art. 19 (registries — open to non-EU entities). Registry Regulation (EU) 2019/1122 Art. 15 (account types). EU CBAM: Regulation (EU) 2023/956 Art. 4 (scope), Art. 7 (CBAM certificates). GBA Framework: Outline Development Plan for GBA (2019) Section 6.3 (financial cooperation). HKEX Quadripartite MoU with Guangzhou EE, Shenzhen Green Exchange, Macao ICE (September 2025). Hong Kong: Securities and Futures Ordinance Cap. 571 Part V (licensing), Schedule 5 (regulated activities — carbon not classified).',
    depth: 'expert' as ContentDepth,
  },
];
...
export const RISK_MITIGATIONS = [
  ...
  { id: 'cpty-risk', risk: 'Counterparty Risk', description: 'NIHA as principal in OTC trades', mitigation: 'Segregated client funds, staged settlement, institutional-grade custody', depth: 'essential' as ContentDepth },
  { id: 'fx-risk', risk: 'FX Risk (EUR/CNY)', description: 'Currency volatility during settlement', mitigation: 'Price locked at trade date minimizes exposure window; FX hedging available for T+2–T+5 settlement period', depth: 'essential' as ContentDepth },
...
export const FAQ_ITEMS: { category: string; id: string; question: string; answer: string; depth: ContentDepth }[] = [
  ...
  { category: 'mechanism', id: 'swap-works', question: 'How does the carbon-credit-to-EUA swap work?', answer: 'Five steps: (1) The EU client deposits EUR into NIHA\'s segregated client account in Hong Kong. (2) NIHA acquires Chinese carbon credits from counterparties on pilot markets through established cross-border settlement channels, paying in RMB. (3) Credits are registered through NIHA\'s institutional market access on the pilot exchange — required because the EU entity cannot hold Chinese carbon credits. (4) NIHA executes the economic swap: retires/sells the Chinese credits and delivers EUA sourced from its counterparty network. (5) EUA is transferred to the EU client\'s European carbon registry account at 8-12% below market price. The entire process settles within T+2 to T+5 business days from trade date — faster than the multi-leg exchange alternative. The EU entity never interacts with Chinese regulation.', depth: 'essential' },
  { category: 'mechanism', id: 'custody-requirement', question: 'Why must NIHA hold carbon credits in custody?', answer: 'Chinese carbon credits exist only in exchange registry accounts — you need a trading membership to hold them. EU entities cannot open accounts on the national ETS (closed by law) or practically on pilot markets (access requires government-facilitated authorization not available to EU entities). NIHA, operating through exclusive GBA arrangements, registers the credits through its institutional market access. Beneficial ownership is documented in the bilateral contract under HK law, but legal title stays with NIHA — mirroring how prime brokers hold securities for clients. NIHA must act as principal, not agent, because the EU entity and Chinese entity operate in mutually exclusive regulatory systems and cannot transact directly.', depth: 'essential' },
  { category: 'mechanism', id: 'replication-barrier', question: 'What prevents someone from replicating NIHA\'s model?', answer: 'Six barriers: (1) Government-facilitated access — the authorization pathway through GBA frameworks takes months to establish and requires ongoing compliance. (2) Bilateral relationships with Chinese carbon credit holders — years to build, trust-based, not transferable. (3) Cross-border regulatory expertise across EU and Chinese carbon law. (4) Swap execution infrastructure and operational track record. (5) Network effects — sellers attract buyers and vice versa, creating a cold-start problem for new entrants. (6) GBA jurisdictional advantage — only Hong Kong entities have the GBA zone access needed, and even among HK entities, the authorization process is a significant barrier.', depth: 'essential' },
  { category: 'compliance', id: 'eu-cant-buy-cea', question: 'Why can\'t EU entities access Chinese carbon credits?', answer: 'Two separate barriers. First, China\'s national ETS (Shanghai SEEE) is closed to ALL foreign entities by State Council regulation — including Hong Kong entities. Second, the pilot markets (Shenzhen, Guangdong/CEEX, Hubei) that do allow foreign investors require: (1) Cross-border settlement capability — months of setup with no existing EU-China banking relationship, (2) Government foreign exchange authorization — no precedent for EU applicants in carbon trading, (3) Institutional market access — untested for EU entities. No EU entity has ever navigated this pathway. NIHA, as a GBA-zone Hong Kong entity, has established access through exclusive government-facilitated arrangements.', depth: 'essential' },
  { category: 'compliance', id: 'legal-sound', question: 'Is this structure legally sound?', answer: 'Yes. NIHA operates through established, regulated pathways verified in each jurisdiction. In China: government-facilitated access for authorized institutional participants on pilot markets (no quota/currency restrictions for qualified GBA entities). In Hong Kong: carbon credits are not regulated as securities under the SFO, and GBA Standard Contracts cover cross-border data compliance. In the EU: European carbon markets are open to non-EU entities under established regulatory frameworks. The price differential exists because EU and Chinese carbon markets are structurally disconnected — not because of any regulatory grey area.', depth: 'essential' },
  ...
  { category: 'pricing', id: 'why-not-direct', question: 'Why wouldn\'t an EU entity just buy EUA directly?', answer: 'They can and most do. But they pay full market price (€81/t) with visible market impact. Via NIHA: same EUA at 8-12% discount, zero market impact (off-exchange, dark pool), faster bilateral settlement (T+2–T+5 vs 5-10d multi-leg), and 24/7 availability. For a 500,000 tonne order, that\'s €4-5M in savings.', depth: 'essential' },
  ...
  { category: 'pricing', id: 'settlement-terms', question: 'What are the settlement terms?', answer: 'NIHA settles T+2 to T+5 from trade date — faster than the multi-leg exchange route (5-10 business days for sell CEA + convert FX + buy EUA). Counterparties commit (reserve) certificate availability before trade execution, so delivery is pre-agreed and price is locked at trade date. Trading terms can be negotiated 24/7, not limited to exchange hours (11h/day for EU ETS, 4h/day for China ETS). Minimum order size is flexible and negotiated bilaterally.', depth: 'essential' },
  // Advanced
  { category: 'compliance', id: 'hk-access-china', question: 'Can Hong Kong entities access China ETS directly?', answer: 'Not the national ETS — that remains closed to all non-mainland entities. However, HK entities can access pilot markets (particularly Shenzhen) through the Greater Bay Area framework. This requires: HK-incorporated entity, established cross-border settlement capability, institutional market access authorization, and compliance with pilot market regulations. NIHA has established this access through government-facilitated GBA arrangements — a process that took months and requires ongoing compliance. This is one of NIHA\'s key structural advantages.', depth: 'advanced' },
  { category: 'getting-started', id: 'competitive-moat', question: 'What is NIHA\'s competitive moat?', answer: 'Five layers: (1) Geographic — HK is the only jurisdiction with simultaneous GBA carbon access and European market infrastructure. (2) Regulatory — government-facilitated access pathways that take months to establish and are not available to entities outside the GBA framework. (3) Relationship — bilateral trust with Chinese CEA holders cannot be replicated quickly. (4) Network — two-sided marketplace creates cold-start problem for competitors. (5) First-mover — established track record and operational expertise compound over time.', depth: 'advanced' },
  // Expert
