...
- **Contact/NDA requests**: POST `/contact/request` and POST `/contact/nda-request` return `ContactRequestResponse` (id, entity_name, contact_email, contact_name, position, nda_file_name, submitter_ip, user_role, request_flow, referred_by_user_id, referral_code_used, notes, created_at). user_role is the role in the flow (NDA, KYC, REJECTED). `request_flow` is `'buyer'` (default) or `'introducer'`. WebSocket `new_request` payload matches this shape. The frontend normalizes API and WebSocket contact-request payloads (camelCase) to snake_case at the realtime hook boundary (`useBackofficeRealtime`) so backoffice code can assume snake_case. All DB writes in `backend/app/api/v1/contact.py` use try/except, rollback, and `handle_database_error` from `app/core/exceptions`. Admin update contact request (`PUT /api/v1/admin/contact-requests/{request_id}`) uses the same error-handling pattern.
