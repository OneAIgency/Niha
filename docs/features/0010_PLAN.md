...
Plan tehnic pentru fluxul complet de onboarding: de la cerere NDA până la status EUA (acces complet). Statusurile sunt unificate pentru `User.role` și `ContactRequest.user_role` unde se aplică.
...
### 1.1 Enum unificat (User.role și ContactRequest.user_role)
...
- La creare user din contact request (endpoint existent `POST /admin/users/create-from-request`):
  ...
  - Setare **contact_request.user_role = KYC** (sau un status care să indice „approved, user created”; verbatim: „clientul devine KYC”).
