...
- **Scope:** Buton Approve pe cereri NDA → modal „Approve & Create User” → `POST /api/v1/admin/users/create-from-request` → user KYC cu entity nou, contact_request.status = KYC; opțional email invitație. Erori afișate corect în UI (standardizat + hint când există).
...
5. **Persistență (în try/except):**
   ...
   - `contact_request.status = ContactStatus.KYC`; `commit`; `refresh(user)`.
...
- Dacă migrările nu sunt aplicate, setarea `contact_request.status = KYC` poate genera „invalid input value for enum” → gestionată ca mai sus.
