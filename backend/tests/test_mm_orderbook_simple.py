...
async def main():
    ...
    async with async_session() as session:
        try:
            ...
            """),
                {"id": mm_id, "name": mm_name, "code": mm_code},
            )

            print("✓ Created market maker:")
            print(f"  ID: {mm_id}")
            print(f"  Name: {mm_name}")
            print("  Type: CASH_BUYER")
            print("  EUR Balance: €100,000.00")
            ...
            if orderbook["bids"]:
                print("\nBid Levels:")
                header = f"  {'Price':>10} | {'Quantity':>10} | {'Orders':>6}"
                print(f"{header} | {'Cumulative':>12}")
                print(f"  {'-' * 10}-+-{'-' * 10}-+-{'-' * 6}-+-{'-' * 12}")
            ...
            for order_data in test_orders_data:
                price_found = any(
                    bid["price"] == order_data["price"] for bid in orderbook["bids"]
                )
                if price_found:
