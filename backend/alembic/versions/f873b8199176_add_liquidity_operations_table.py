...
def upgrade() -> None:
    ...
    certificatetype = postgresql.ENUM('EUA', 'CEA', name='certificatetype', create_type=False)
    ...
    op.create_table(
        ...
        sa.Column('target_bid_liquidity_eur', sa.Numeric(precision=18, scale=2), nullable=False),
        sa.Column('target_ask_liquidity_eur', sa.Numeric(precision=18, scale=2), nullable=False),
        sa.Column('actual_bid_liquidity_eur', sa.Numeric(precision=18, scale=2), nullable=False),
        sa.Column('actual_ask_liquidity_eur', sa.Numeric(precision=18, scale=2), nullable=False),
        sa.Column('market_makers_used', postgresql.JSONB(astext_type=sa.Text()), nullable=False),
