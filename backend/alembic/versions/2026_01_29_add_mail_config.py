...
def upgrade():
    ...
    # Create enum type only if it does not exist (idempotent; app init_db may have created it)
    conn.execute(sa.text(
        "DO $$ BEGIN CREATE TYPE mailprovider AS ENUM ('resend', 'smtp'); "
        "EXCEPTION WHEN duplicate_object THEN NULL; END $$;"
    ))

    # Create table only if it does not exist (idempotent; app init_db may have created it)
    ...
    op.create_table(
        ...
        sa.Column("provider", mail_provider_enum, nullable=False, server_default="resend"),
        sa.Column("use_env_credentials", sa.Boolean(), nullable=False, server_default=sa.true()),
        ...
        sa.Column("invitation_token_expiry_days", sa.Integer(), nullable=False, server_default=sa.text("7")),
