...
class SettlementProcessor:
            ...
            for settlement in settlements:
                if SettlementProcessor._should_advance_status(settlement):
                    ...
                    if next_status:
                        ...
                        # Send email notification to all users of the entity
    ...
    async def _send_status_update_emails(
        ...
        """Send email notifications to all users of the entity for settlement status update"""
        try:
            ...
            if not entity:
                logger.warning(f"No entity found for settlement {settlement.batch_reference}")
                return
            ...
            if not users:
                logger.info(f"No users found for entity {entity.legal_name} - skipping email notification")
                return
            ...
            for user in users:
                if user.email:
                    try:
                        ...
                        logger.info(
                            f"Sent settlement status update email to {user.email} "
                            f"for {settlement.batch_reference}"
                        )
            ...
            logger.error(f"Error sending status update emails for {settlement.batch_reference}: {e}")
