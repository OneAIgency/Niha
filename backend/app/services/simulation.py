...
class SimulationEngine:
    ...
    def _get_price_variance(
        self, base_price: float, variance_pct: float = 0.05
    ) -> float:
            ...
            swaps.append(
                {
                    ...
                    "status": random.choices(["open", "matched"], weights=[0.8, 0.2])[
                        0
                    ],
                    "created_at": listing_time.isoformat(),
    ...
    def get_market_stats(self) -> Dict:
        ...
        return {
            ...
            "avg_cea_price": round(
                sum(s["unit_price"] for s in cea_sellers) / len(cea_sellers), 4
            ),
            "avg_eua_price": round(
                sum(s["unit_price"] for s in eua_sellers) / len(eua_sellers), 4
            ),
